...
<head>
    ...
    <title>DHMR | Command Center</title>
...
<body class="flex h-screen overflow-hidden">
    ...
    <!-- Sidebar -->
    <div class="w-1/3 glass-panel z-10 flex flex-col shadow-2xl">
        <div class="p-8 border-b border-white/5 flex justify-between items-center">
            <div>
                ...
                <p class="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Lagos Operational Sector</p>
            </div>
    ...
    <script>
        ...
        function renderAlerts(alerts) {
            ...
            alerts.forEach(alert => {
                ...
                let contactInfo = alert.contacts.map(c => `<div><p class="font-bold text-white">${c.contact_name}</p><p class="opacity-50 text-[10px]">${c.contact_phone}</p></div>`).join('');

                html += `
                    <div class="alert-card p-6 rounded-3xl ${isHighRisk ? 'pulse-red' : ''}">
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <h3 class="font-bold text-white text-lg tracking-tight">${alert.name}</h3>
                                <p class="text-[10px] text-gray-500 uppercase tracking-widest mt-1">${new Date(alert.created_at).toLocaleTimeString()}</p>
                            </div>
                            <div class="px-3 py-1 rounded-full text-[10px] font-bold ${isHighRisk ? 'bg-red-500 text-white' : 'bg-yellow-500 text-black'}">
                                RISK ${alert.risk_score}
                            </div>
                        </div>
                        <div class="space-y-4 mb-6">
                            <p class="text-[10px] uppercase tracking-widest font-bold opacity-30">Emergency Contacts</p>
                            <div class="grid grid-cols-2 gap-4 text-xs">${contactInfo}</div>
                        </div>
                        <button onclick="resolveAlert(${alert.id})" class="w-full bg-white text-black py-3 rounded-2xl text-[10px] font-bold uppercase tracking-widest hover:bg-gray-200 transition">
                            Resolve Event
                        </button>
                    </div>
                `;
